...
def fit_hier(
    ...
    """
    Hierarchical model fitting (EM) to estimate group-level priors.
    ...
    Args:
        ...
        tol: Convergence tolerance for the group-level loop (stops when improvement < tol).
        max_iter: Maximum number of group-level iterations.
        init: Optional initialization for group parameters, e.g. `(mus, vs)`.
        verbose: If True, prints progress per subject and iteration.
    ...
    while True:
        ...
        for i, (_, item) in enumerate(fit_info.items()):
            ...
            try:
                ...
                l = item["log_post"] + 0.5 * (n_param * np.log(2 * np.pi) - log_h)
                gh = 1
            except Exception:
...
def fit(
    ...
    Args:
        ...
        p_priors: a list of scipy random variable, used to
                calculate log prior
    ...
    if alg == "bads":
        ...
        bads = BADS(loss_fn_bads, param0, lb, ub, plb, pub, options={"display": "off"})
    ...
    if verbose:
        print(f"  Fitted params: {x_min}, Loss: {f_min}")
...
def fit_bms(
    ...
    Args:
        ...
        tol: tolerence for the convergence
    Outputs:
    ...
    if use_bic:
        lme = np.vstack([-(0.5) * np.array(fit_info["bic"]) for fit_info in all_sub_info]).T
    else:
...
def calc_lme(fit_info):
    ...
    for s in range(len(fit_info["log_post"])):
        ...
        l = fit_info["log_post"][s] + 0.5 * (
            fit_info["n_param"] * np.log(2 * np.pi) - h
        )
        lme.append(l)
...
def FE(lme, p_m1D, alpha_post, alpha0):
    ...
    E_log_rmD = (
        (p_m1D * (lme + E_log_r)).sum()
        + ((alpha0 - 1) * E_log_r).sum()
        + gammaln(alpha0.sum())
        - (gammaln(alpha0)).sum()
    )
